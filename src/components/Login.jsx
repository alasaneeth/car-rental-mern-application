...
const Login = ({ setShowLogin }) => {
    ...
    return (
        <div onClick={() => { setShowLogin(false) }} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex-items-center text-sm text-gra-600 bg-black/50'>
            <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
                ...
                <button 
                    type="submit"
                    disabled={loading}
                    className="bg-indigo-500 hover:bg-indigo-600 transition-all text-white w-full py-2 rounded-md cursor-pointer disabled:bg-indigo-300 disabled:cursor-not-allowed"
                >
